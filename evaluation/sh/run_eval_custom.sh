...
DATASET_NAME="math-500"  # You can modify this as per your requirement
...
for MODEL_NAME_OR_PATH in "${MODEL_NAME_LIST[@]}"; do
    ...
    bash sh/custom_eval.sh "$PROMPT_TYPE" "$MODEL_NAME_OR_PATH" "$DATASET_NAME" 2>&1 | tee -a "$LOG_FILE"
done
